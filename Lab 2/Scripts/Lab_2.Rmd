...
date: "`r Sys.Date()`"
output: html_document #when you knit it will make it a nice way to share script 
editor_options: 
  chunk_output_type: console
---

## #yammel header-tell it what to do

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE)

#giving default settings, will always appear at beginning 
```
...
```{r}
# Your code here:
bird_wingspan_cm <- 105
bird_wingspan_m <- bird_wingspan_cm/100
```
...
class(population_size)  # "numeric"- to figure would what the class the data is 
```
...
```{r}
#character always uses quotes (can also show up with "" around number too)
genus <- "Carcharhinus"
...
5 < 3         # FALSE
5 == 5 # TRUE (note: double equals for comparison) you need double equals for equal values. single can be used to assign variables 
5 != 3        # TRUE (5 is NOT equal to 3) (!= is not equal)
...
weight <- NA
is.na(weight)  # TRUE - check if something is NA; if you have in the dataset 
...
as.numeric("hello")  # NA (with a warning) cannot change 
```
...
```{r}
# Your code here:
fav_animal<- "orcinus orca" 
lifespan <- 50
is_extinct <- FALSE #have to give it the statement and determine if true or false

class("orinus orca")
class(lifespan)
class(is_extinct)

as.character(lifespan)
lifespan <- as.character(lifespan)
```
...
paste("Sample", 1, sep="_")                # "Sample_1" sep ==to seperate it with what is after 
...
toupper("hello")           # "HELLO" convert to uppercase
tolower("WORLD")           # "world" convert to lowercase
substr("Accipiter", 1, 3)  # Extract characters: "Acc" ;the first 3
```
...
```{r}
# Numeric vector (e.g., body masses of 5 birds in grams): c==combine
...
rep(c("A", "B"), times=3)       # "A", "B", "A", "B", "A", "B" ==alternate
rep(c("A", "B"), each=3)        # "A", "A", "A", "B", "B", "B" ==in a row
```
...
body_mass[body_mass > 1300] # Only values > 1300: 1350, 1425; indexing out what meets this criteria

# Which birds are above average weight?
mean_mass <- mean(body_mass)
body_mass[body_mass > mean_mass] #what is bigger in set than the mean 
```
...
# Element-wise operations between vectors: only when you have the same amount of values in vector
...
```{r}
# Your code here:
temps <- c(18.5, 20.2, 19.8, 22.1, 21.5, 19.0, 17.8)
mean_temp <- mean(temps)
mean_temp

temps[temps > 20] 
which (temps >20) #what position in the vector are those values
range(temps)
```
...
# View the data frame: or as view()
birds
```
...
dim(birds)          # Both, dimensions: always rows then columns 5 4 
...
```{r}
# Method 1: $ notation (most common) -THIS IS THE BEST WAY 
...
```{r}
birds[1, ]              # First row (all columns from the blank space after comma)
birds[c(1, 3), ]        # Rows 1 and 3 and all columns associated
...
# Or use negative for numeric columns: EASIER WAY 
birds[order(-birds$body_mass_g), ]
```
...
# Your code here:
trees <- data.frame( 
tree.species = c("Oak", "Maple", "Pine", "Birch"),
tree.height.m = c(25, 20, 30, 18),
is_evergreen= c(FALSE, FALSE, TRUE, FALSE)
)

trees$tree.height.ft <- trees$tree.height.m*3.28
trees[trees$tree.height.m > 20, ]
#use which 


mean(trees$tree.height.m)
median(trees$tree.height.m)
summary(trees)
```
...
complete.cases(measurements)          # TRUE/FALSE for each row; FALSE means that they have NAs in this function 
```
...
sum(is.na(measurements$length))
median(is.na(measurements$length))
