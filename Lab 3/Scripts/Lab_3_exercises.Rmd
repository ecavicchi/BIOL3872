...
new_df <- cbind(iris$Sepal.Length, iris$Sepal.Width)
#column binding
class(new_df)
...
hist(iris$Sepal.Length, col = "maroon", breaks = 50, xlab="Sepal Length (cm)", main = "Iris data")
...
#don't really need this section ;) but fun to have; think about using
# R will read | as or 
...
#virginica will still show up because species is still a factor of 3
```
...
ggplot(data = iris, aes(x = Petal.Length, y = Sepal.Length, color = Species))+
  #make sure color is included in the aes if wanting to color by group
  geom_point()
#can add fill function to fill all points in the geom_ function paraenthesis 
...
#line directly above is overriding the aesthetic given before, wouldn't reccommend
```
...
#violin is good for showing frequency and amount of variation 
```
...
iris %>% 
  ...
  ggplot(aes(x=Species, y= Petal.Length, color= Species, size = 0.5))+ 
  geom_violin()
```
:::
