...
troops_p <- ggplot(Minard.troops, aes(long, lat)) +
  geom_path(aes(size = survivors, color = direction,
                group = group))
#advance and retreat different colors and survivors dictates how thick the line is (size)
...
\
Now, polish the annotations of city positions along the *Grand Armée* path of travel.
...
temp_p <- ggplot(data = Minard.temp, aes(x = long, y = temp, color = temp))+
  ...
  scale_color_gradientn(colors = (oce.colorsTemperature(20)),
                        name = "Temperature")
#scale graident is showing rnages changing and color cooresponds to avergae temp at time documented
temp_p
```
...
#plot_layout is the patch them together
Napoleons_retreat
```
...
ggplot(mtcars2, aes(hp, mpg, color = am)) +  geom_point() + geom_smooth() +
  #geom_smooth is showing bad relationship and not a good call to plot against 
  theme(legend.position = 'bottom') + 
   labs (
    x = "Horsepower",
    y = "Miles per gallon",
...
Choosing a smoother is somewhat arbitrary, importantly they're only capturing patterns you have, meaning their predictive power is not great.
...
```{r}
p_no_data <- ggplot(datasets, aes(x = x, y = y)) +
...
```{r}
p_with_data <- ggplot(datasaurus_dozen, aes(x = x, y = y)) +
...
```{r}
#ink heavy- more dnese than neccessary 
...
#ask sophie to go back to that bc lost 

cars_to_label <- c()
mtcars2 %>% 
  rownames_to_column(var = "cars") %>% 
  filter((car)%in% cars_to_label) %>% 
...
#group one is sequential, good for shades 
#second is qualitative pallette 
#third is divergent pallette (for color blindness)
```
...
Using this ggplot2 to plot car weight versus miles per gallon using the built-in mtcars which maps the number of cylinders to point colour and facet the plot by the number of gears, use a colour palette of your choosing with scale_color_manual(), or other palettes you feel good about! Should you be using a discrete or continuous palette?
