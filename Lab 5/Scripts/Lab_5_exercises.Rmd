...
if(x>10) {
  print("larger than ten")
    ...
    print("is ten")
    } else if(x<0) {print("is a negative")}
...
#if the variable is numeric it will print if not it will make it and then print it 
if (is.numeric(x)) {
  print("x is numeric")
} else {
  x <- as.numeric(x)
...
#for each element in the vector make new one called z, "i" doensn't matter 
#z is being overridden everytime why you get one variable 
```
...
```{r}
fish_abundance <- read.csv("~/Desktop/BIOL3872/Lab 5/Data/Groundfish_surveys.csv")

hist(fish_abundance$depth)
#can add log to standardize 

fish_abundance %>% 
  mutate(year = sort(factor(year, levels = as.character(2016:2020)))) %>% 
  # Remove so log transform doesn't produce negative values
  filter(num_cpue > 0) %>% 
#remove anything with catch per unit effort below zero
#the pipe this new filtered set into a ggplot
  ggplot(aes(x = log(num_cpue), y = depth, color = year))+
  geom_point(alpha = 0.5) + 
  scale_y_reverse() +
#to make depth make more sense (surface at top), also able to reverse x but why would you??
  theme_bw(12)+
  scale_color_brewer(palette = "Spectral") +
  guides(color = guide_legend(override.aes = list(alpha = 1, size = 4)))
#last line shows points in the legend easier 
...
#unique is used to pull out all unique values in that category

# This is a list! Like a vector but more flexible (can store multiple data types)
Depths_output <- list()
#lists are simialr to vectors and cause it to not be overrideen with multiple types
...
speciesunique <- unique(groundfishes$Taxon)
Depth <- unique(groundfishes$Depth)

fill <- list()

for (i in speciesunique) {
  speciesdata <- groundfishes[groundfishes$Taxon == i,]
  for (d in Depth) {
    depthdata <- groundfishes[groundfishes$Depth == d]
    summed <- sum()
#not done. look at photo
...
task4 <- left_join(abundance_by_year, depth_by_species, by= "accepted_name")
task4
