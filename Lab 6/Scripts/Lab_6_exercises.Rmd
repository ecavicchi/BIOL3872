...
```{r}
#month.abb for month abbreviations! 
...
as.Date("13-02/2012", format = "%d-%m/%Y")
#keep consistent and then it will be reported correct
...
as.Date("February-26-2026", format = "%B-%d-%Y")

as.Date("Jul/29/01", format= "%b/%y/%d")

as.Date("Dec-9-95", format= "%b-%d-%y")

as.Date("December/4/13 19:11:31", format= "%B/%d/%y %H:%M:%S")

as.Date("December-19-10 9:30:49pm", format= "%B-%d-%y %I:%M:%S%p")
...
#modify the format of the time that is given (keeping 12hr clock) 
format(ex_datetime, format = "%B-%d-%y %I:%M:%S%p")
#OlsonNames() will be useful 
...
```{r}
Bedford_ts <- read.csv("~/Desktop/BIOL3872/Lab 6/Data/BedfordBasin_ts.csv"); Bedford_ts = Bedford_ts[,-1] # Remove row name duplicate column
```
...
#can use month.name for full name 
Bedford_ts$Month_name <- month.abb[Bedford_ts$Month]

Bedford_Date= as.Date(make_datetime(year=Bedford_ts$Year,
                                    month=Bedford_ts$Month,
                                    day= Bedford_ts$Day))

#?identical is also a possibility
all(Bedford_ts$Date==Bedford_Date)
```
...
#can subtract and greater than or less than 
```
