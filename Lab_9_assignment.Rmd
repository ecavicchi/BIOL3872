...
### Filter Data for start and end points

```{r}
first_point <- my_data[which.min(my_data$date),]
last_point <- my_data[which.max(my_data$date),]
```
...
my_data %>% mutate(month_name = factor(month_name, levels = month.abb))%>%
  ...
  geom_point(data=first_point, aes(x=longitude, y=latitude), shape=17, size=4.5, color= "green", alpha=0.5)+
  
  # Layer 6: End point (red square)
  geom_point(data=last_point, aes(x=longitude, y=latitude), shape=15, size=4.5, color="red", alpha=0.5) +
  ...
  values = c("Jan" = "#313695", "Feb" = "#4575b4", "Mar" = "#74add1",
             ...
             "Jul" = "#fdae61", "Aug" = "#e7400d", "Sep" = "#d73027",
